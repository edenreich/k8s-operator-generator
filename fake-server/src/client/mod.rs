    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
        ...
        context: &C) -> Result<CatsGetResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<Vec<models::Cat>>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(CatsGetResponse::OK
                    (body)
                )
            }
            code => {
        ...
        context: &C) -> Result<CatsIdPutResponse, ApiError>
    {
        ...
        let body = serde_json::to_string(&param_cat).expect("impossible to fail to serialize");
        ...
        context: &C) -> Result<DogsGetResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<Vec<models::Dog>>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(DogsGetResponse::OK
                    (body)
                )
            }
            code => {
        ...
        context: &C) -> Result<DogsIdPutResponse, ApiError>
    {
        ...
        let body = serde_json::to_string(&param_dog).expect("impossible to fail to serialize");
                *request.body_mut() = Body::from(body);
