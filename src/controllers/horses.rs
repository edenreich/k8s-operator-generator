    ...
    match event {
        WatchEvent::Added(mut horse) => {
            ...
            if horse.metadata.deletion_timestamp.is_some() {
                ...
                handle_delete_horse_by_id(&config, &mut horse, kubernetes_api).await;
                return;
            }
...
pub async fn handle_delete_horse_by_id(
    ...
    if let Err(e) = delete_horse_by_id(&config, &uuid).await {
        error!("Failed to delete a horse by id: {:?}", e);
        return;
    }
...
pub async fn handle_update_horse_by_id(
    ...
    if let Err(e) = update_horse_by_id(&config, &uuid, dto).await {
        error!("Failed to update a horse by id: {:?}", e);
        return;
    }
    ...
    match kubernetes_api
        ...
        .await
    {
        Ok(_) => info!("update a horse by id went successfully"),
        Err(e) => error!("Failed to update a horse by id: {:?}", e),
    }
}
